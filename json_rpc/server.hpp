...
public:
    ...
    int run(Func&& fn)
    {
        _transporter.run([this,fn = std::forward<Func>(fn)](const json& data) -> future<json>{
            if (!data.is_object())
            {
                OUTPUT_STREAM << "Invalid JSON, expected an object or array" << std::endl;
                return boost::make_exceptional(std::invalid_argument("Invalid JSON, expected on object or array"));
            }
            auto method_json = data["method"];
            if (!method_json.is_string())
            {
                OUTPUT_STREAM << "Invalid body: 'method' must be a string\n";
                return boost::make_exceptional(std::invalid_argument("Invalid body: 'method' must be a string."));
            }
            ...
            return fn(method, params).then([data](future<json> result) -> future<json> {
